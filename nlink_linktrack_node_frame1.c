...
void unpackNodeFrame1Data(uint8_t *byteArray) {
    ...
    for(int i=0;i<nodeFrame1Data_.framePart.validNodeQuantity;++i){
        RawNode1_t rawNode;
        memcpy(&rawNode,byteArray + framePartLength + i*sizeof(RawNode1_t),sizeof(RawNode1_t));
        ...
        int24ToFloat(rawNode.posTemp, nodeFrame1Data_.node[rawNode.id]->pos,3,kPosMultiply_);
    }

    nodeFrame1Data_.supplyVoltage = nodeFrame1Data_.framePart.supplyVoltageTemp/kVoltageMultiply_;
